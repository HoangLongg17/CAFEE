        ...
        private void dgvHoaDon_SelectionChanged(object sender, EventArgs e)
        {
            if (dgvHoaDon.CurrentRow != null && dgvHoaDon.CurrentRow.DataBoundItem != null)
            {
                ...
                try
                {
                    ...
                    // (SỬA LẠI CHỖ NÀY)
                    // Thay vì gọi GetChiTietHoaDon, gọi hàm GetHoaDonDayDu
                    HoaDonDayDuDTO dtoDayDu = lichSuBUS.GetHoaDonDayDu(maHD);

                    // Lấy danh sách Items (đã gộp cả bán và tặng) từ DTO
                    if (dtoDayDu != null)
                    {
                        // Đổ chi tiết lên grid TRÊN
                        dgvNhanVien.DataSource = dtoDayDu.Items;
                    }
                    else
                    {
                        dgvNhanVien.DataSource = null;
                    ...
                    // Đổi tên cột grid trên (hàm này vẫn giữ nguyên)
